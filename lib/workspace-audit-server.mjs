...
export function createWorkspaceAuditServer({ rootDir, publicDir }) {
  ...
  function normalizeConvergenceAssimilationContractRunner(value) {
    const runner = toText(value).toLowerCase();
    return runner === "claude" ? "claude" : "codex";
  }

  function buildConvergenceAssimilationCliHandoffContractMarkdown(payload) {
    const gate = toControlPlaneRecord(payload.readinessGate);
    const schema = toControlPlaneRecord(payload.expectedResultSchema);
    const lines = [
      "# Convergence Assimilation CLI Handoff Contract",
      "",
      `Generated: ${new Date().toLocaleString()}`,
      `Runner: ${toText(payload.runner) || "codex"}`,
      `Readiness decision: ${toText(gate.decision) || "review"}`,
      `Execution mode: ${toText(payload.executionMode) || "operator-supervised"}`,
      `Secret policy: ${toText(payload.secretPolicy) || "Non-secret CLI handoff only."}`,
      "",
      "## Instructions",
      `- ${toText(payload.runner) || "codex"} must use the latest copied convergence assimilation run trace pack, result ledger, and readiness gate before changing code.`,
      "- Execute one bounded implementation slice only.",
      "- Return structured, non-secret result metadata to Workspace Audit Pro using the result intake form.",
      "- Do not paste credentials, tokens, certificates, private keys, cookies, browser sessions, or raw secret-bearing logs.",
      "",
      "## Expected Result Schema",
      `- status: ${toTextList(schema.status).join(" | ")}`,
      `- summary: ${toText(schema.summary)}`,
      `- changedFiles: ${toText(schema.changedFiles)}`,
      `- validationSummary: ${toText(schema.validationSummary)}`,
      `- blockers: ${toText(schema.blockers)}`,
      `- nextAction: ${toText(schema.nextAction)}`,
      "",
      "## Readiness Gate",
      gate.markdown || "Readiness gate markdown not available."
    ];

    return lines.join("\n");
  }

  function createConvergenceAssimilationCliHandoffContractPayload(persisted, options = {}) {
    const runner = normalizeConvergenceAssimilationContractRunner(options.runner);
    const readinessGate = createConvergenceAssimilationReadinessGatePayload(persisted);
    const payload = {
      generatedAt: new Date().toISOString(),
      protocolVersion: "convergence-assimilation-cli-handoff-contract.v1",
      runner,
      executionMode: "operator-supervised-non-secret-cli-handoff",
      readinessGate,
      expectedResultSchema: {
        status: ["passed", "failed", "blocked", "needs-review", "cancelled"],
        summary: "Short non-secret outcome summary.",
        changedFiles: "Array of repo-relative paths only; no file contents or secrets.",
        validationSummary: "Short validation command/result summary without raw secret-bearing output.",
        blockers: "Array of non-secret blocker summaries.",
        nextAction: "Recommended next operator or runner action."
      },
      recommendedAction: readinessGate.decision === "ready"
        ? `Copy this contract into ${runner} with the selected trace pack and result ledger before executing the next bounded convergence assimilation slice.`
        : `Do not execute yet. Resolve readiness gate decision ${readinessGate.decision} before handing off to ${runner}.`,
      secretPolicy: "Non-secret convergence assimilation CLI handoff contract only. Do not store credentials, tokens, certificates, private keys, cookies, browser sessions, raw command output, or private repository access material."
    };

    return {
      ...payload,
      markdown: buildConvergenceAssimilationCliHandoffContractMarkdown(payload)
    };
  }

  function buildConvergenceAssimilationOperatorPlaybookMarkdown(payload) {
    const steps = Array.isArray(payload.steps) ? payload.steps.map(toControlPlaneRecord) : [];
    const lines = [
      "# Convergence Assimilation Operator Playbook",
      "",
      `Generated: ${new Date().toLocaleString()}`,
      `Audience: ${toText(payload.audience) || "vibe coder / operator"}`,
      `Secret policy: ${toText(payload.secretPolicy) || "Non-secret operating guide only."}`,
      "",
      "## Operating Cycle"
    ];

    for (const step of steps) {
      lines.push(`- ${toText(step.title) || "Step"}: ${toText(step.detail) || "Review Workspace Audit Pro before continuing."}`);
    }

    lines.push("", "## Guardrails");
    for (const guardrail of toTextList(payload.guardrails)) {
  ...
  function createConvergenceAssimilationOperatorPlaybookPayload(persisted) {
    const readinessGate = createConvergenceAssimilationReadinessGatePayload(persisted);
    const payload = {
      generatedAt: new Date().toISOString(),
      protocolVersion: "convergence-assimilation-operator-playbook.v1",
      audience: "vibe coder / operator supervising Codex CLI and Claude CLI",
      readinessGateDecision: readinessGate.decision,
      steps: [
        {
          title: "1. Review the readiness gate",
          detail: "Open Governance -> Convergence Assimilation Readiness Gate. Continue only when the decision is ready, or resolve review/hold reasons first."
        },
        {
          title: "2. Copy the runner contract",
          detail: "Use Copy Codex Contract or Copy Claude Contract to give the selected CLI a bounded, non-secret instruction package."
        },
        {
          title: "3. Copy the trace pack",
          detail: "Use Copy Trace Pack on the selected assimilation run so the runner sees the pair, draft, validation commands, and related tasks."
        },
        {
          title: "4. Run one bounded implementation slice",
          detail: "Let the CLI change only the requested slice. Avoid broad refactors unless the contract explicitly calls for them."
        },
        {
          title: "5. Validate and relaunch",
          detail: "Run the validation commands, rebuild static preview, relaunch the local app, and check the affected UI/API endpoints."
        },
        {
          title: "6. Record the result",
          detail: "Use Record Result on the run card with a short non-secret summary, validation summary, changed files, blockers, and next action."
        },
        {
          title: "7. Checkpoint the result",
          detail: "Confirm, Defer, or Escalate the result in Governance. Confirmed evidence can feed the next build slice; deferred or escalated items become follow-up work."
        }
      ],
      guardrails: [
        "Never paste credentials, tokens, certificates, private keys, cookies, browser sessions, or raw secret-bearing command output into the app.",
        "Use one bounded runner contract per implementation slice.",
        "Record every runner outcome before queueing the next assimilation pass.",
        "Treat failed, blocked, or escalated records as hold conditions until reviewed.",
        "Keep the app as the control center; Codex CLI and Claude CLI are workers, not the source of truth."
      ],
      recommendedAction: readinessGate.decision === "ready"
        ? "Use this playbook with the CLI handoff contract for the next bounded convergence build."
        : "Resolve readiness gate review or hold items before executing the next convergence build.",
      secretPolicy: "Non-secret convergence assimilation operator playbook only. Do not store credentials, tokens, certificates, private keys, cookies, browser sessions, raw command output, or private repository access material."
    };

    return {
      ...payload,
      markdown: buildConvergenceAssimilationOperatorPlaybookMarkdown(payload)
    };
  }

  function buildConvergenceAssimilationSessionPacketMarkdown(payload) {
    const sections = [
      "# Convergence Assimilation Session Packet",
      "",
      `Generated: ${new Date().toLocaleString()}`,
      `Runner: ${toText(payload.runner) || "codex"}`,
      `Secret policy: ${toText(payload.secretPolicy) || "Non-secret session packet only."}`,
      "",
      "## Recommended Action",
      toText(payload.recommendedAction) || "Review the included packet before executing a convergence assimilation session.",
      "",
      "## Readiness Gate",
      toText(toControlPlaneRecord(payload.readinessGate).markdown),
      "",
      "## Operator Playbook",
      toText(toControlPlaneRecord(payload.operatorPlaybook).markdown),
      "",
      "## CLI Handoff Contract",
      toText(toControlPlaneRecord(payload.cliHandoffContract).markdown),
      "",
      "## Run Ledger",
      toText(toControlPlaneRecord(payload.runLedger).markdown),
      "",
      "## Result Ledger",
      toText(toControlPlaneRecord(payload.resultLedger).markdown),
      "",
      "## Result Checkpoint Ledger",
      toText(toControlPlaneRecord(payload.resultCheckpointLedger).markdown)
    ];

    return sections.join("\n");
  }

  function createConvergenceAssimilationSessionPacketPayload(persisted, options = {}) {
    const runner = normalizeConvergenceAssimilationContractRunner(options.runner);
    const readinessGate = createConvergenceAssimilationReadinessGatePayload(persisted);
    const payload = {
      generatedAt: new Date().toISOString(),
      protocolVersion: "convergence-assimilation-session-packet.v1",
      runner,
      readinessGate,
      operatorPlaybook: createConvergenceAssimilationOperatorPlaybookPayload(persisted),
      cliHandoffContract: createConvergenceAssimilationCliHandoffContractPayload(persisted, { runner }),
      runLedger: createConvergenceAssimilationRunLedgerPayload(persisted, { status: "all" }),
      resultLedger: createConvergenceAssimilationResultLedgerPayload(persisted, { status: "all" }),
      resultCheckpointLedger: createConvergenceAssimilationResultCheckpointLedgerPayload(persisted, { status: "all" }),
      recommendedAction: readinessGate.decision === "ready"
        ? `Use this ${runner} session packet to execute one bounded convergence assimilation build cycle.`
        : `Resolve readiness gate decision ${readinessGate.decision} before executing this ${runner} session packet.`,
      secretPolicy: "Non-secret convergence assimilation session packet only. Do not store credentials, tokens, certificates, private keys, cookies, browser sessions, raw command output, or private repository access material."
    };

    return {
      ...payload,
      markdown: buildConvergenceAssimilationSessionPacketMarkdown(payload)
    };
  }

  function createConvergenceAssimilationSessionPacketSnapshotRecord(persisted, options = {}) {
    const packet = createConvergenceAssimilationSessionPacketPayload(persisted, {
      runner: options.runner
    });
    const readinessSummary = toControlPlaneRecord(packet.readinessGate?.summary);

    return {
      id: createId("convergence-assimilation-session-packet-snapshot"),
      title: toText(options.title) || `Convergence Assimilation ${packet.runner} Session Packet`,
      runner: packet.runner,
      protocolVersion: packet.protocolVersion,
      packetGeneratedAt: packet.generatedAt,
      readinessDecision: toText(packet.readinessGate?.decision) || "review",
      recommendedAction: packet.recommendedAction,
      runCount: toNumber(readinessSummary.runCount),
      openRunCount: toNumber(readinessSummary.openRunCount),
      resultCount: toNumber(readinessSummary.resultCount),
      checkpointCount: toNumber(readinessSummary.checkpointCount),
      openCheckpointCount: toNumber(readinessSummary.openCheckpointCount),
      secretPolicy: packet.secretPolicy,
  ...
  function buildConvergenceAssimilationSessionPacketSnapshotDiffMarkdown(payload) {
    const lines = [
    ...
    const driftItems = Array.isArray(payload.driftItems) ? payload.driftItems : [];
    if (!driftItems.length) {
      lines.push("- No convergence assimilation session packet drift detected.");
      return lines.join("\n");
    }
    for (const item of driftItems) {
      const record = toControlPlaneRecord(item);
      lines.push(`- ${toText(record.label) || toText(record.field) || "Session packet drift"}: ${toText(record.before)} -> ${toText(record.current)}`);
    }
    return lines.join("\n");
  }

  function createMissingConvergenceAssimilationSessionPacketSnapshotDiffPayload() {
    const payload = {
      generatedAt: new Date().toISOString(),
      hasSnapshot: false,
      snapshotId: "",
      ...
      driftScore: 0,
      driftSeverity: "missing-snapshot",
      recommendedAction: "Save a convergence assimilation session packet snapshot before comparing CLI handoff drift.",
      driftItems: [],
      liveSummary: null,
      snapshotSummary: null
    };
    return {
      ...payload,
      markdown: buildConvergenceAssimilationSessionPacketSnapshotDiffMarkdown(payload)
    };
  }

  function createConvergenceAssimilationSessionPacketSnapshotDiffPayload(snapshot, livePacket, persisted = {}) {
    const snapshotRecord = toControlPlaneRecord(snapshot);
    const liveSummary = toControlPlaneRecord(livePacket.readinessGate?.summary);
    const snapshotSummary = {
      readinessDecision: toText(snapshotRecord.readinessDecision) || "review",
      runCount: toNumber(snapshotRecord.runCount),
      openRunCount: toNumber(snapshotRecord.openRunCount),
      resultCount: toNumber(snapshotRecord.resultCount),
      checkpointCount: toNumber(snapshotRecord.checkpointCount),
      openCheckpointCount: toNumber(snapshotRecord.openCheckpointCount),
      recommendedAction: toText(snapshotRecord.recommendedAction)
    };
    const currentSummary = {
      readinessDecision: toText(livePacket.readinessGate?.decision) || "review",
      runCount: toNumber(liveSummary.runCount),
      openRunCount: toNumber(liveSummary.openRunCount),
      resultCount: toNumber(liveSummary.resultCount),
      checkpointCount: toNumber(liveSummary.checkpointCount),
      openCheckpointCount: toNumber(liveSummary.openCheckpointCount),
      recommendedAction: toText(livePacket.recommendedAction)
    };
    const driftFields = [
      { field: "readinessDecision", label: "Readiness decision" },
      { field: "runCount", label: "Run count" },
      { field: "openRunCount", label: "Open run count" },
      { field: "resultCount", label: "Result count" },
      { field: "checkpointCount", label: "Checkpoint count" },
      { field: "openCheckpointCount", label: "Open checkpoint count" },
    ...
    const driftItems = driftFields
      .map(({ field, label }) => ({
        field,
        label,
        before: snapshotSummary[field],
        current: currentSummary[field],
        delta: typeof currentSummary[field] === "number" ? toNumber(currentSummary[field]) - toNumber(snapshotSummary[field]) : 0
      }))
      .filter((item) => String(item.before) !== String(item.current));

    const normalizePacketMarkdown = (value) => toText(value)
      .split("\n")
      .filter((line) => !line.startsWith("Generated:"))
      .join("\n");
    const snapshotMarkdown = normalizePacketMarkdown(snapshotRecord.markdown);
    const liveMarkdown = normalizePacketMarkdown(livePacket.markdown);
    if (snapshotMarkdown && liveMarkdown && snapshotMarkdown !== liveMarkdown) {
      driftItems.push({
        field: "markdown",
        label: "Packet Markdown body",
        before: `${snapshotMarkdown.length} chars`,
        current: `${liveMarkdown.length} chars`,
        delta: 1
      });
    }

    const checkpointTasks = (Array.isArray(persisted.tasks) ? persisted.tasks : [])
      .map(toControlPlaneRecord)
      .filter((task) => toText(task.sourceType) === "convergence-assimilation-session-packet-snapshot-drift-checkpoint"
        && toText(task.convergenceAssimilationSessionPacketSnapshotId) === toText(snapshotRecord.id));
    const checkpointByField = new Map(checkpointTasks.map((task) => [toText(task.convergenceAssimilationSessionPacketDriftField), task]));
    const decoratedDriftItems = driftItems.map((item) => {
      const checkpoint = checkpointByField.get(toText(item.field));
      return checkpoint
        ? {
            ...item,
            checkpointTaskId: toText(checkpoint.id),
            checkpointDecision: toText(checkpoint.convergenceAssimilationSessionPacketDriftDecision),
            checkpointStatus: toText(checkpoint.status),
            checkpointedAt: toText(checkpoint.convergenceAssimilationSessionPacketDriftCheckpointedAt)
          }
        : item;
    });
    const driftScore = decoratedDriftItems.reduce((total, item) => total + Math.max(1, Math.abs(toNumber(item.delta))), 0);
    const driftSeverity = !decoratedDriftItems.length
      ? "none"
      : decoratedDriftItems.some((item) => item.field === "readinessDecision" || item.field === "openRunCount" || item.field === "openCheckpointCount")
        ? "high"
        : driftScore >= 5
          ? "medium"
          : "low";
    const recommendedAction = driftSeverity === "high"
      ? "Refresh the session packet snapshot before handing work to Codex or Claude."
      : driftSeverity === "medium"
        ? "Review packet drift and refresh the snapshot if the live handoff has changed intentionally."
        : driftSeverity === "low"
          ? "Monitor low session packet drift and refresh the packet after the next checkpoint."
          : "No convergence assimilation session packet drift detected.";
    const payload = {
      generatedAt: new Date().toISOString(),
      hasSnapshot: true,
      snapshotId: toText(snapshotRecord.id),
      snapshotTitle: toText(snapshotRecord.title) || "Convergence Assimilation Session Packet",
      snapshotCreatedAt: toText(snapshotRecord.createdAt),
      runner: toText(snapshotRecord.runner) || toText(livePacket.runner) || "codex",
      hasDrift: driftItems.length > 0,
      driftScore,
      driftSeverity,
      recommendedAction,
      driftItems: decoratedDriftItems,
      liveSummary: currentSummary,
      snapshotSummary
    };
    return {
      ...payload,
  ...
  function getConvergenceAssimilationSessionPacketDriftCheckpointDecision(decision) {
  ...
  function createConvergenceAssimilationSessionPacketDriftCheckpointTask(snapshot, driftItem, decision, note = "") {
    const snapshotRecord = toControlPlaneRecord(snapshot);
    const item = toControlPlaneRecord(driftItem);
    const decisionInfo = getConvergenceAssimilationSessionPacketDriftCheckpointDecision(decision);
    const now = new Date().toISOString();
    return {
      id: createId("convergence-assimilation-session-packet-drift-checkpoint"),
      title: `${decisionInfo.label}: ${toText(item.label) || toText(item.field) || "Session packet drift"}`,
      description: [
        `Snapshot: ${toText(snapshotRecord.title) || toText(snapshotRecord.id) || "Convergence Assimilation Session Packet"}`,
        `Runner: ${toText(snapshotRecord.runner) || "codex"}`,
        `Field: ${toText(item.label) || toText(item.field)}`,
        `Before: ${toText(item.before)}`,
        `Current: ${toText(item.current)}`,
        note ? `Note: ${note}` : ""
      ].filter(Boolean).join("\n"),
      status: decisionInfo.status,
      priority: decisionInfo.priority,
      projectId: "convergence-assimilation-control",
      projectName: "Convergence Assimilation Control",
      sourceType: "convergence-assimilation-session-packet-snapshot-drift-checkpoint",
      convergenceAssimilationSessionPacketSnapshotId: toText(snapshotRecord.id),
      convergenceAssimilationSessionPacketSnapshotTitle: toText(snapshotRecord.title),
      convergenceAssimilationSessionPacketRunner: toText(snapshotRecord.runner) || "codex",
      convergenceAssimilationSessionPacketDriftField: toText(item.field),
      convergenceAssimilationSessionPacketDriftLabel: toText(item.label),
      convergenceAssimilationSessionPacketDriftDecision: decisionInfo.key,
      convergenceAssimilationSessionPacketDriftSeverity: toText(item.field) === "readinessDecision" ? "high" : "review",
      convergenceAssimilationSessionPacketDriftBefore: toText(item.before),
      convergenceAssimilationSessionPacketDriftCurrent: toText(item.current),
      convergenceAssimilationSessionPacketDriftDelta: toNumber(item.delta),
      convergenceAssimilationSessionPacketDriftCheckpointedAt: now,
      convergenceAssimilationSessionPacketDriftCheckpointNote: note,
      secretPolicy: "non-secret-convergence-assimilation-session-packet-drift-checkpoint-only",
      createdAt: now,
      updatedAt: now
    };
  }

  function buildConvergenceAssimilationSessionPacketDriftCheckpointLedgerMarkdown(payload) {
    const summary = toControlPlaneRecord(payload.summary);
    const lines = [
      "# Convergence Assimilation Session Packet Drift Checkpoint Ledger",
      "",
      `Generated: ${new Date().toLocaleString()}`,
      `Filter: ${toText(payload.status) || "all"}`,
      `Secret policy: ${toText(payload.secretPolicy) || "Non-secret convergence assimilation session packet drift checkpoint metadata only."}`,
      "",
      "## Summary",
      `- Total checkpoints: ${toNumber(summary.total)}`,
      `- Visible checkpoints: ${toNumber(summary.visible)}`,
      `- Open checkpoints: ${toNumber(summary.open)}`,
      `- Closed checkpoints: ${toNumber(summary.closed)}`,
      `- Confirmed: ${toNumber(summary.confirmed)}`,
      `- Deferred: ${toNumber(summary.deferred)}`,
      `- Escalated: ${toNumber(summary.escalated)}`,
      "",
    ...
    const items = Array.isArray(payload.items) ? payload.items : [];
    if (!items.length) {
      lines.push("- No session packet drift checkpoints matched this filter.");
      return lines.join("\n");
    }
    for (const item of items.slice(0, 80)) {
      const record = toControlPlaneRecord(item);
      lines.push(`- ${toText(record.title) || toText(record.id) || "Session packet drift checkpoint"}: ${toText(record.convergenceAssimilationSessionPacketDriftDecision) || "deferred"} / ${toText(record.status) || "open"}`);
      lines.push(`  Snapshot: ${toText(record.convergenceAssimilationSessionPacketSnapshotTitle) || toText(record.convergenceAssimilationSessionPacketSnapshotId) || "not recorded"} | Runner: ${toText(record.convergenceAssimilationSessionPacketRunner) || "codex"}`);
      lines.push(`  Field: ${toText(record.convergenceAssimilationSessionPacketDriftLabel) || toText(record.convergenceAssimilationSessionPacketDriftField) || "not recorded"}`);
      lines.push(`  Before -> current: ${toText(record.convergenceAssimilationSessionPacketDriftBefore)} -> ${toText(record.convergenceAssimilationSessionPacketDriftCurrent)}`);
      if (toText(record.convergenceAssimilationSessionPacketDriftCheckpointNote)) {
        lines.push(`  Note: ${toText(record.convergenceAssimilationSessionPacketDriftCheckpointNote)}`);
      }
    }
    return lines.join("\n");
  }

  function createConvergenceAssimilationSessionPacketDriftCheckpointLedgerPayload(persisted, options = {}) {
    const requestedStatus = toText(options.status) || "all";
    const status = ["all", "open", "closed"].includes(requestedStatus) ? requestedStatus : "all";
    const allItems = (Array.isArray(persisted.tasks) ? persisted.tasks : [])
      .map(toControlPlaneRecord)
    ...
    const summary = {
      ...
      open: allItems.filter((item) => !isGovernanceClosedStatus(item.status)).length,
      closed: allItems.filter((item) => isGovernanceClosedStatus(item.status)).length,
      confirmed: allItems.filter((item) => toText(item.convergenceAssimilationSessionPacketDriftDecision) === "confirmed").length,
      deferred: allItems.filter((item) => toText(item.convergenceAssimilationSessionPacketDriftDecision) === "deferred").length,
      escalated: allItems.filter((item) => toText(item.convergenceAssimilationSessionPacketDriftDecision) === "escalated").length
    };
    const payload = {
      generatedAt: new Date().toISOString(),
      status,
      summary,
      items: visibleItems.slice(0, 100),
      secretPolicy: "Non-secret convergence assimilation session packet drift checkpoint metadata only. Do not store credentials, tokens, certificates, private keys, cookies, browser sessions, raw command output, or private repository access material."
    };
    return {
      ...payload,
      markdown: buildConvergenceAssimilationSessionPacketDriftCheckpointLedgerMarkdown(payload)
    };
  }

  function buildConvergenceAssimilationRunnerCommandQueueDraftMarkdown(payload) {
    const lines = [
      "# Convergence Assimilation Runner Command Queue Draft",
      "",
      `Generated: ${new Date().toLocaleString()}`,
      `Runner: ${toText(payload.runner) || "codex"}`,
      `Execution mode: ${toText(payload.executionMode) || "operator-supervised-non-executing-draft"}`,
      `Secret policy: ${toText(payload.secretPolicy) || "Non-secret runner command queue only."}`,
      "",
      "## Recommended Action",
      toText(payload.recommendedAction) || "Review this draft before running any CLI commands.",
      "",
      "## Queue"
    ];
    const commands = Array.isArray(payload.commands) ? payload.commands : [];
    if (!commands.length) {
      lines.push("- No runner command draft items available.");
      return lines.join("\n");
    }
    for (const command of commands) {
      const record = toControlPlaneRecord(command);
      lines.push(`- ${toText(record.title) || toText(record.id) || "Runner step"}`);
      lines.push(`  Purpose: ${toText(record.purpose) || "Review before execution."}`);
      lines.push(`  Command hint: ${toText(record.commandHint) || "manual step"}`);
      lines.push(`  Guardrail: ${toText(record.guardrail) || "Do not paste credentials or raw private logs."}`);
    }
    return lines.join("\n");
  }

  function createConvergenceAssimilationRunnerCommandQueueDraftPayload(persisted, options = {}) {
    const runner = normalizeConvergenceAssimilationContractRunner(options.runner);
    const sessionPacket = createConvergenceAssimilationSessionPacketPayload(persisted, { runner });
    const readinessGate = toControlPlaneRecord(sessionPacket.readinessGate);
    const readinessDecision = toText(readinessGate.decision) || "review";
    const repoRoot = process.cwd();
    const commands = [
      {
        id: "review-readiness-gate",
        title: "Review readiness gate",
        purpose: "Confirm the current convergence assimilation gate before handing work to a CLI runner.",
        commandHint: "Copy the session packet and review the Readiness Gate section.",
        guardrail: "Do not proceed on hold; resolve review items first unless explicitly supervised."
      },
      {
        id: `open-${runner}-cli`,
        title: `Open ${runner} CLI in repository root`,
        purpose: "Start the selected runner in the app repository after reviewing the packet.",
        commandHint: `cd ${repoRoot}`,
        guardrail: "Open the CLI manually; Workspace Audit Pro is only drafting the queue at this stage."
      },
      {
        id: "paste-session-packet",
        title: "Paste the session packet as runner context",
        purpose: "Give the runner the readiness gate, playbook, contract, run ledger, result ledger, and checkpoints.",
        commandHint: `Paste the copied ${runner} session packet into ${runner} CLI.`,
        guardrail: "Only paste non-secret packet content generated by this app."
      },
      {
        id: "execute-one-bounded-build",
        title: "Execute one bounded convergence build",
        purpose: "Limit the runner to one implementation slice tied to the packet objective.",
        commandHint: "Ask the runner to implement one packet-scoped build, then stop for validation.",
        guardrail: "Do not let the runner modify unrelated apps or secret-bearing files."
      },
      {
        id: "validate-and-relaunch",
        title: "Validate and relaunch",
        purpose: "Run the local validation stack and relaunch the app for monitoring after the build.",
        commandHint: "Run parser, tests, static build, diff hygiene, then restart node server.mjs.",
        guardrail: "Capture validation summaries, not raw logs with private paths or credentials."
      },
      {
        id: "record-result",
        title: "Record runner result",
        purpose: "Feed the non-secret runner result back into Workspace Audit Pro.",
        commandHint: "Use the convergence assimilation result intake controls or API.",
        guardrail: "Summarize changed files, validation, blockers, and next action only."
      }
    ];
    const payload = {
      generatedAt: new Date().toISOString(),
      protocolVersion: "convergence-assimilation-runner-command-queue-draft.v1",
      runner,
      executionMode: "operator-supervised-non-executing-draft",
      readinessDecision,
      readinessGate,
      sessionPacketRecommendedAction: sessionPacket.recommendedAction,
      commands,
      recommendedAction: readinessDecision === "ready"
        ? `Use this ${runner} queue draft to run one supervised convergence assimilation build.`
        : `Resolve readiness gate decision ${readinessDecision} before executing this ${runner} command queue.`,
      secretPolicy: "Non-secret convergence assimilation runner command queue draft only. Do not store credentials, tokens, certificates, private keys, cookies, browser sessions, raw command output, or private repository access material."
    };
    return {
      ...payload,
      markdown: buildConvergenceAssimilationRunnerCommandQueueDraftMarkdown(payload)
    };
  }

  function buildConvergenceAssimilationRunnerResultReplayChecklistMarkdown(payload) {
    const lines = [
      "# Convergence Assimilation Runner Result Replay Checklist",
      "",
      `Generated: ${new Date().toLocaleString()}`,
      `Runner: ${toText(payload.runner) || "codex"}`,
      `Secret policy: ${toText(payload.secretPolicy) || "Non-secret result replay checklist only."}`,
      "",
      "## Replay Rules"
    ];
    const items = Array.isArray(payload.items) ? payload.items : [];
    if (!items.length) {
      lines.push("- No replay checklist items available.");
      return lines.join("\n");
    }
    for (const item of items) {
      const record = toControlPlaneRecord(item);
      lines.push(`- [ ] ${toText(record.title) || toText(record.id) || "Replay step"}`);
      lines.push(`  Evidence: ${toText(record.evidence) || "Non-secret summary only."}`);
      lines.push(`  App action: ${toText(record.appAction) || "Record or checkpoint in Workspace Audit Pro."}`);
    }
    return lines.join("\n");
  }

  function createConvergenceAssimilationRunnerResultReplayChecklistPayload(persisted, options = {}) {
    const runner = normalizeConvergenceAssimilationContractRunner(options.runner);
    const readinessGate = createConvergenceAssimilationReadinessGatePayload(persisted);
    const resultLedger = createConvergenceAssimilationResultLedgerPayload(persisted, { status: "all" });
    const checkpointLedger = createConvergenceAssimilationResultCheckpointLedgerPayload(persisted, { status: "all" });
    const items = [
      {
        id: "summarize-runner-output",
        title: "Summarize runner output without raw logs",
        evidence: "One paragraph: what changed, why, and whether the runner stayed inside the session packet.",
        appAction: "Use result intake summary; do not paste command transcripts, tokens, cookies, certificates, or private keys."
      },
      {
        id: "list-changed-files",
        title: "List changed files and ownership boundaries",
        evidence: "Relative paths and a short reason for each changed file.",
        appAction: "Record changed files in convergence assimilation result intake."
      },
      {
        id: "capture-validation-summary",
        title: "Capture validation summary",
        evidence: "Parser, test, static build, diff hygiene, and relaunch smoke status.",
        appAction: "Paste concise pass/fail summaries into validation summary fields."
      },
      {
        id: "record-blockers",
        title: "Record blockers or access constraints",
        evidence: "Only non-secret blocker labels and next required access method.",
        appAction: "Mark result blocked or needs-review when credentials, certificates, permissions, or private repos are required."
      },
      {
        id: "checkpoint-result",
        title: "Checkpoint the captured result",
        evidence: "Confirm, defer, or escalate the result based on validation and operator review.",
        appAction: "Use convergence assimilation result checkpoint controls."
      },
      {
        id: "refresh-session-packet",
        title: "Refresh packet snapshot after accepted replay",
        evidence: "Saved packet snapshot and drift status after replay.",
        appAction: "Save a new Codex or Claude packet snapshot if result/checkpoint state changed."
      }
    ];
    const payload = {
      generatedAt: new Date().toISOString(),
      protocolVersion: "convergence-assimilation-runner-result-replay-checklist.v1",
      runner,
      readinessDecision: readinessGate.decision,
      resultSummary: resultLedger.summary,
      checkpointSummary: checkpointLedger.summary,
      items,
      recommendedAction: readinessGate.decision === "ready"
        ? `Use this ${runner} replay checklist after the next bounded runner session.`
        : `Resolve readiness gate decision ${readinessGate.decision} before replaying new ${runner} results.`,
      secretPolicy: "Non-secret convergence assimilation runner result replay checklist only. Do not store credentials, tokens, certificates, private keys, cookies, browser sessions, raw command output, or private repository access material."
    };
    return {
      ...payload,
      markdown: buildConvergenceAssimilationRunnerResultReplayChecklistMarkdown(payload)
    };
  }

  function buildConvergenceAssimilationRunnerLaunchpadGateMarkdown(payload) {
    const lines = [
      "# Convergence Assimilation Runner Launchpad Gate",
      "",
      `Generated: ${new Date().toLocaleString()}`,
      `Runner: ${toText(payload.runner) || "codex"}`,
      `Decision: ${toText(payload.decision) || "review"}`,
      `Recommended action: ${toText(payload.recommendedAction) || "Review launchpad readiness."}`,
      `Secret policy: ${toText(payload.secretPolicy) || "Non-secret launchpad gate only."}`,
      "",
      "## Reasons"
    ];
    const reasons = Array.isArray(payload.reasons) ? payload.reasons : [];
    if (!reasons.length) {
      lines.push("- No launchpad blockers detected.");
      return lines.join("\n");
    }
    for (const reason of reasons) {
      const record = toControlPlaneRecord(reason);
      lines.push(`- ${toText(record.severity) || "review"}: ${toText(record.message) || toText(record.code) || "Launchpad review required."}`);
    }
    return lines.join("\n");
  }

  function createConvergenceAssimilationRunnerLaunchpadGatePayload(persisted, options = {}) {
    const runner = normalizeConvergenceAssimilationContractRunner(options.runner);
    const readinessGate = createConvergenceAssimilationReadinessGatePayload(persisted);
    const snapshots = Array.isArray(persisted.convergenceAssimilationSessionPacketSnapshots)
      ? persisted.convergenceAssimilationSessionPacketSnapshots.map(toControlPlaneRecord)
      : [];
    const latestRunnerSnapshot = snapshots.find((snapshot) => toText(snapshot.runner) === runner) || snapshots[0] || null;
    const packetDrift = latestRunnerSnapshot
      ? createConvergenceAssimilationSessionPacketSnapshotDiffPayload(latestRunnerSnapshot, createConvergenceAssimilationSessionPacketPayload(persisted, { runner: toText(latestRunnerSnapshot.runner) || runner }), persisted)
      : createMissingConvergenceAssimilationSessionPacketSnapshotDiffPayload();
    const driftCheckpointLedger = createConvergenceAssimilationSessionPacketDriftCheckpointLedgerPayload(persisted, { status: "all" });
    const reasons = [];
    if (readinessGate.decision === "hold") {
      reasons.push({ severity: "hold", code: "readiness-hold", message: "Convergence assimilation readiness gate is on hold." });
    } else if (readinessGate.decision !== "ready") {
      reasons.push({ severity: "review", code: "readiness-review", message: `Convergence assimilation readiness gate is ${readinessGate.decision}.` });
    }
    if (!packetDrift.hasSnapshot) {
      reasons.push({ severity: "review", code: "missing-session-packet-snapshot", message: "Save a session packet snapshot before launch." });
    } else if (packetDrift.hasDrift && packetDrift.driftSeverity === "high") {
      reasons.push({ severity: "hold", code: "high-session-packet-drift", message: "High session packet drift requires refresh or checkpoint review before launch." });
    } else if (packetDrift.hasDrift) {
      reasons.push({ severity: "review", code: "session-packet-drift", message: "Session packet drift is present; review before launch." });
    }
    if (toNumber(driftCheckpointLedger.summary.escalated) > 0) {
      reasons.push({ severity: "hold", code: "escalated-packet-drift-checkpoints", message: "Escalated packet drift checkpoints are open for operator action." });
    }
    if (toNumber(driftCheckpointLedger.summary.open) > 0) {
      reasons.push({ severity: "review", code: "open-packet-drift-checkpoints", message: "Open packet drift checkpoints should be resolved before launch." });
    }
    ...
    const payload = {
      generatedAt: new Date().toISOString(),
      protocolVersion: "convergence-assimilation-runner-launchpad-gate.v1",
      runner,
      decision,
      readinessGate,
      packetDrift,
      driftCheckpointSummary: driftCheckpointLedger.summary,
      reasons,
      recommendedAction: decision === "ready"
        ? `Launch one supervised ${runner} convergence assimilation build using the latest session packet and command queue.`
        : decision === "hold"
          ? "Do not launch a runner session until hold reasons are resolved."
          : "Review launchpad reasons, refresh packet snapshots if needed, then relaunch the gate.",
      secretPolicy: "Non-secret convergence assimilation runner launchpad gate only. Do not store credentials, tokens, certificates, private keys, cookies, browser sessions, raw command output, or private repository access material."
    };
    return {
      ...payload,
      markdown: buildConvergenceAssimilationRunnerLaunchpadGateMarkdown(payload)
    };
  }
   ...
   *   convergenceAssimilationSessionPacketSnapshots?: Array<Record<string, unknown>>,
  ...
  function buildGovernanceSnapshot(persisted, inventory, options = {}) {
    ...
    const convergenceAssimilationSessionPacketSnapshots = Array.isArray(persisted.convergenceAssimilationSessionPacketSnapshots) ? persisted.convergenceAssimilationSessionPacketSnapshots : [];
    ...
    const convergenceTaskLedgerSnapshotFocus = [...convergenceTaskLedgerSnapshots]
      .sort((left, right) => new Date(toText(right.createdAt)).getTime() - new Date(toText(left.createdAt)).getTime())
      .slice(0, 24);
    const convergenceAssimilationSessionPacketSnapshotFocus = [...convergenceAssimilationSessionPacketSnapshots]
      .sort((left, right) => new Date(toText(right.createdAt)).getTime() - new Date(toText(left.createdAt)).getTime())
      .slice(0, 24);
    ...
    const summary = {
      ...
      convergenceAssimilationSessionPacketSnapshotCount: convergenceAssimilationSessionPacketSnapshots.length,
    ...
    const governanceSnapshot = {
      ...
      convergenceAssimilationSessionPacketSnapshots: convergenceAssimilationSessionPacketSnapshotFocus,
  ...
  return createServer(async (req, res) => {
    ...
    if (req.method === "GET" && pathname === "/api/convergence/assimilation-cli-handoff-contract") {
      try {
        const persisted = await store.readStore();
        return sendJson(res, 200, createConvergenceAssimilationCliHandoffContractPayload(persisted, {
          runner: requestUrl.searchParams.get("runner")
        }));
      } catch (error) {
        return sendJson(res, 500, { error: error.message });
    ...
    if (req.method === "GET" && pathname === "/api/convergence/assimilation-operator-playbook") {
      try {
        const persisted = await store.readStore();
        return sendJson(res, 200, createConvergenceAssimilationOperatorPlaybookPayload(persisted));
      } catch (error) {
        return sendJson(res, 500, { error: error.message });
      }
    }

    if (req.method === "GET" && pathname === "/api/convergence/assimilation-session-packet") {
      try {
        const persisted = await store.readStore();
        return sendJson(res, 200, createConvergenceAssimilationSessionPacketPayload(persisted, {
          runner: requestUrl.searchParams.get("runner")
        }));
      } catch (error) {
        return sendJson(res, 500, { error: error.message });
    ...
    if (req.method === "GET" && pathname === "/api/convergence/assimilation-session-packet-snapshots/diff") {
      try {
        const persisted = await store.readStore();
        const snapshots = Array.isArray(persisted.convergenceAssimilationSessionPacketSnapshots)
          ? persisted.convergenceAssimilationSessionPacketSnapshots.map(toControlPlaneRecord)
        ...
        const requestedRunner = normalizeConvergenceAssimilationContractRunner(requestUrl.searchParams.get("runner"));
        const snapshotPool = snapshotId === "latest"
          ? snapshots.filter((item) => toText(item.runner) === requestedRunner)
          : snapshots;
        const snapshot = snapshotId === "latest"
          ? snapshotPool[0] || snapshots[0]
          : snapshots.find((item) => toText(item.id) === snapshotId);
        if (!snapshot) {
          return sendJson(res, 200, createMissingConvergenceAssimilationSessionPacketSnapshotDiffPayload());
        }
        const livePacket = createConvergenceAssimilationSessionPacketPayload(persisted, {
          runner: toText(snapshot.runner) || requestedRunner
        });
        return sendJson(res, 200, createConvergenceAssimilationSessionPacketSnapshotDiffPayload(snapshot, livePacket, persisted));
      } catch (error) {
        return sendJson(res, 500, { error: error.message });
      }
    }

    if (req.method === "GET" && pathname === "/api/convergence/assimilation-session-packet-drift-checkpoint-ledger") {
      try {
        const persisted = await store.readStore();
        return sendJson(res, 200, createConvergenceAssimilationSessionPacketDriftCheckpointLedgerPayload(persisted, {
          status: requestUrl.searchParams.get("status")
        }));
      } catch (error) {
        return sendJson(res, 500, { error: error.message });
    ...
    if (req.method === "GET" && pathname === "/api/convergence/assimilation-runner-command-queue-draft") {
      try {
        const persisted = await store.readStore();
        return sendJson(res, 200, createConvergenceAssimilationRunnerCommandQueueDraftPayload(persisted, {
          runner: requestUrl.searchParams.get("runner")
        }));
      } catch (error) {
        return sendJson(res, 500, { error: error.message });
    ...
    if (req.method === "GET" && pathname === "/api/convergence/assimilation-runner-result-replay-checklist") {
      try {
        const persisted = await store.readStore();
        return sendJson(res, 200, createConvergenceAssimilationRunnerResultReplayChecklistPayload(persisted, {
          runner: requestUrl.searchParams.get("runner")
        }));
      } catch (error) {
        return sendJson(res, 500, { error: error.message });
    ...
    if (req.method === "GET" && pathname === "/api/convergence/assimilation-runner-launchpad-gate") {
      try {
        const persisted = await store.readStore();
        return sendJson(res, 200, createConvergenceAssimilationRunnerLaunchpadGatePayload(persisted, {
          runner: requestUrl.searchParams.get("runner")
        }));
      } catch (error) {
        return sendJson(res, 500, { error: error.message });
    ...
    if (req.method === "POST" && pathname === "/api/convergence/assimilation-session-packet-snapshot-drift-checkpoints") {
      try {
        const body = await readRequestBody(req);
        const payload = body ? JSON.parse(body) : {};
        ...
        const requestedRunner = normalizeConvergenceAssimilationContractRunner(payload.runner);
        const snapshotPool = snapshotId === "latest"
          ? snapshots.filter((item) => toText(item.runner) === requestedRunner)
          : snapshots;
        const snapshot = snapshotId === "latest"
          ...
          : snapshots.find((item) => toText(item.id) === snapshotId);
        if (!snapshot) {
          return sendJson(res, 400, { error: "Convergence assimilation session packet snapshot not found" });
        }
        const diff = createConvergenceAssimilationSessionPacketSnapshotDiffPayload(snapshot, createConvergenceAssimilationSessionPacketPayload(persisted, {
          runner: toText(snapshot.runner) || requestedRunner
        }), persisted);
        ...
        if (!driftItem) {
          return sendJson(res, 400, { error: "Session packet drift item not found" });
        }
        const decisionInfo = getConvergenceAssimilationSessionPacketDriftCheckpointDecision(payload.decision);
        const note = toText(payload.note);
        const existingIndex = (Array.isArray(persisted.tasks) ? persisted.tasks : [])
          .findIndex((task) => toText(task.sourceType) === "convergence-assimilation-session-packet-snapshot-drift-checkpoint"
            && toText(task.convergenceAssimilationSessionPacketSnapshotId) === toText(snapshot.id)
            && toText(task.convergenceAssimilationSessionPacketDriftField) === toText(driftItem.field));
        const checkpointTask = createConvergenceAssimilationSessionPacketDriftCheckpointTask(snapshot, driftItem, decisionInfo.key, note);
        const updated = await store.updateStore((current) => {
          ...
          if (existingIndex >= 0) {
            currentTasks[existingIndex] = {
              ...currentTasks[existingIndex],
              ...checkpointTask,
              id: currentTasks[existingIndex].id,
              createdAt: currentTasks[existingIndex].createdAt || checkpointTask.createdAt,
              updatedAt: checkpointTask.updatedAt
            };
          } else {
            currentTasks.unshift(checkpointTask);
          }
          return appendGovernanceOperations({
            ...current,
            tasks: currentTasks
            ...
            createGovernanceOperation(
              "convergence-assimilation-session-packet-drift-checkpoint-upserted",
              `${decisionInfo.label} convergence assimilation session packet drift ${toText(driftItem.label) || toText(driftItem.field)}.`,
              {
                snapshotId: toText(snapshot.id),
                field: toText(driftItem.field),
                decision: decisionInfo.key,
        ...
        return sendJson(res, 200, {
          ...
          mode: existingIndex >= 0 ? "updated" : "created",
          decision: decisionInfo.key,
          task: existingIndex >= 0 ? updated.tasks[existingIndex] : checkpointTask,
          tasks: updated.tasks
        });
      } catch (error) {
        return sendJson(res, 400, { error: error.message || "Invalid convergence assimilation session packet drift checkpoint request" });
      }
    }

    if (pathname === "/api/convergence/assimilation-session-packet-snapshots") {
      if (req.method === "GET") {
        try {
          const persisted = await store.readStore();
          const snapshots = Array.isArray(persisted.convergenceAssimilationSessionPacketSnapshots)
            ? persisted.convergenceAssimilationSessionPacketSnapshots
            : [];
      ...
      if (req.method === "POST") {
        try {
          ...
          const payload = body ? JSON.parse(body) : {};
          const persisted = await store.readStore();
          const snapshot = createConvergenceAssimilationSessionPacketSnapshotRecord(persisted, payload);
          const updated = await store.updateStore((current) => appendGovernanceOperations({
            ...current,
            convergenceAssimilationSessionPacketSnapshots: [
              snapshot,
              ...(Array.isArray(current.convergenceAssimilationSessionPacketSnapshots) ? current.convergenceAssimilationSessionPacketSnapshots : [])
            ].slice(0, 100)
          }, [
            createGovernanceOperation(
              "convergence-assimilation-session-packet-snapshot-created",
              `Saved Convergence Assimilation ${snapshot.runner} session packet snapshot ${snapshot.title}.`,
              {
                snapshotId: snapshot.id,
                runner: snapshot.runner,
                readinessDecision: snapshot.readinessDecision,
          ...
          return sendJson(res, 200, {
            success: true,
            snapshot,
            convergenceAssimilationSessionPacketSnapshots: updated.convergenceAssimilationSessionPacketSnapshots
          });
        } catch (error) {
          return sendJson(res, 400, { error: error.message || "Invalid convergence assimilation session packet snapshot request" });
        }
      }
    }
