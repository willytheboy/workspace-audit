 ...
 *   agentWorkOrderSnapshots?: Array<Record<string, unknown>>,
  ...
  *   agentExecutionSlaLedgerSnapshots?: Array<Record<string, unknown>>,
   ...
   * @param {{ dataSourcesAccessGate?: Record<string, unknown> | null, dataSourcesAccessReviewQueue?: Record<string, unknown> | null, dataSourcesAccessValidationRunbook?: Record<string, unknown> | null, dataSourcesAccessValidationEvidenceCoverage?: Record<string, unknown> | null }} [options]
   */
  ...
  /**
   * @param {ReturnType<typeof buildGovernanceSnapshot>} governance
   * @param {{ limit?: number, dataSourcesAccessGate?: Record<string, unknown> | null, dataSourcesAccessReviewQueue?: Record<string, unknown> | null, dataSourcesAccessValidationRunbook?: Record<string, unknown> | null }} options
   */
  ...
  /**
   * @param {ReturnType<typeof buildGovernanceSnapshot>} governance
   * @param {{ dataSourcesAccessGate?: Record<string, unknown> | null, dataSourcesAccessReviewQueue?: Record<string, unknown> | null, dataSourcesAccessValidationRunbook?: Record<string, unknown> | null }} options
   */
  ...
  /**
   * @param {{ generatedAt: string, summary: Record<string, unknown>, targets: Array<Record<string, unknown>> }} payload
   */
  ...
  async function createDeploymentHealthPayload() {
    ...
    const payload = {
      ...
      summary: {
        ...
        protectedLikely: targets.filter((target) => target.protectedLikely).length,
        providerCounts
      },
      targets
    };
  ...
  async function createGovernanceSnapshotWithDataSources(persisted, inventory) {
    const [dataSourcesAccessGate, dataSourcesAccessReviewQueue, dataSourcesAccessValidationRunbook] = await Promise.all([
  ...
  return createServer(async (req, res) => {
    ...
    if (req.method === "GET" && pathname === "/api/agent-control-plane") {
      try {
        const persisted = await store.readStore();
        const inventory = await getInventoryPayload();
        const { governance, dataSourcesAccessGate, dataSourcesAccessReviewQueue, dataSourcesAccessValidationRunbook } = await createGovernanceSnapshotWithDataSources(persisted, inventory);
    ...
    if (req.method === "POST" && pathname === "/api/deployments/smoke-check") {
      try {
        ...
        const updated = await store.updateStore((current) => appendGovernanceOperations(current, [
          createGovernanceOperation(
