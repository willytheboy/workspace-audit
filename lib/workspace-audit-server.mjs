...
export function createWorkspaceAuditServer({ rootDir, publicDir }) {
  ...
  function createConvergenceAssimilationSessionPacketSnapshotDiffPayload(snapshot, livePacket) {
    ...
    const driftSeverity = !driftItems.length
      ? "none"
      : driftItems.some((item) => item.field === "readinessDecision" || item.field === "openRunCount" || item.field === "openCheckpointCount")
        ? "high"
        : driftScore >= 5
          ? "medium"
          : "low";
  ...
  return createServer(async (req, res) => {
    ...
    if (req.method === "GET" && pathname === "/api/convergence/assimilation-session-packet-snapshots/diff") {
      try {
        ...
        return sendJson(res, 200, createConvergenceAssimilationSessionPacketSnapshotDiffPayload(snapshot, livePacket));
      } catch (error) {
        return sendJson(res, 500, { error: error.message });
      }
    }
