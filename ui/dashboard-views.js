 ...
 *     checkpointConvergenceAssimilationResult: (resultId: string, payload: { decision: "confirmed" | "deferred" | "escalated", note?: string, activeProjectId?: string, scopeMode?: "project" | "portfolio" }) => Promise<{ success: true, mode: "created" | "updated", decision: string, task: import("./dashboard-types.js").PersistedTask, tasks: import("./dashboard-types.js").PersistedTask[], governanceOperationCount: number }>,
 ...
 *     createConvergenceReviewTasks: (payload?: { pairIds?: string[], pairId?: string, candidates?: import("./dashboard-types.js").ConvergenceCandidate[], status?: "confirmed-overlap" | "needs-review" | "merge-candidate" | "actionable", activeProjectId?: string, scopeMode?: "project" | "portfolio" }) => Promise<{ success: true, requested: number, createdTasks: import("./dashboard-types.js").PersistedTask[], skipped: Array<{ pairId: string, label: string, reason: string }>, totals: { requested: number, created: number, skipped: number }, tasks: import("./dashboard-types.js").PersistedTask[] }>,
 ...
 *     createConvergenceTaskLedgerDriftCheckpoint: (payload: { snapshotId?: string, field: string, decision: "confirmed" | "deferred" | "escalated", activeProjectId?: string, scopeMode?: "project" | "portfolio" }) => Promise<{ success: true, mode: "created" | "updated", decision: string, decisionLabel: string, task: import("./dashboard-types.js").PersistedTask, tasks: import("./dashboard-types.js").PersistedTask[] }>,
...
export function createDashboardViews({ getData, getState, getRuntime, api, openModal }) {
  ...
  function applyGovernanceFilters(governance) {
    ...
    function matchesExecutionStatus(run) {
      ...
      if (executionStatus === "alert-baseline-review") return (run.regressionAlertTaskLedgerBaselineHealth || "missing") !== "healthy" || (run.regressionAlertTaskLedgerBaselineFreshness || "missing") !== "fresh" || (run.regressionAlertTaskLedgerBaselineRefreshGateDecision || "ready") !== "ready" || (run.regressionAlertTaskLedgerBaselineUncheckpointedDriftCount || 0) > 0 || (run.regressionAlertTaskLedgerBaselineOpenEscalatedCheckpointCount || 0) > 0;
      if (executionStatus === "alert-baseline-missing") return (run.regressionAlertTaskLedgerBaselineHealth || "missing") === "missing";
      if (executionStatus === "alert-baseline-stale") return (run.regressionAlertTaskLedgerBaselineHealth || "missing") === "stale" || (run.regressionAlertTaskLedgerBaselineFreshness || "missing") === "stale";
      if (executionStatus === "alert-baseline-drift") return ["drifted", "drift-review-required", "changed"].includes(run.regressionAlertTaskLedgerBaselineHealth || "missing") || (run.regressionAlertTaskLedgerBaselineUncheckpointedDriftCount || 0) > 0;
      if (executionStatus === "alert-baseline-hold") return (run.regressionAlertTaskLedgerBaselineRefreshGateDecision || "ready") === "hold" || (run.regressionAlertTaskLedgerBaselineOpenEscalatedCheckpointCount || 0) > 0;
    ...
    const filtered = {
      ...
      regressionAlertTaskLedgerSnapshotDiff: governance.regressionAlertTaskLedgerSnapshotDiff && matchesSearch([
        "regression alert remediation task ledger snapshot drift",
        governance.regressionAlertTaskLedgerSnapshotDiff.snapshotTitle || "",
        governance.regressionAlertTaskLedgerSnapshotDiff.driftSeverity || "",
        governance.regressionAlertTaskLedgerSnapshotDiff.recommendedAction || "",
        governance.regressionAlertTaskLedgerSnapshotDiff.hasDrift ? "drift" : "no drift",
        ...(governance.regressionAlertTaskLedgerSnapshotDiff.driftItems || []).map((item) => `${item.label || ""} ${item.field || ""} ${item.before ?? ""} ${item.current ?? ""} ${item.delta ?? ""}`)
      ])
        ? governance.regressionAlertTaskLedgerSnapshotDiff
        : null,
      regressionAlertTaskLedgerDriftCheckpointLedger: governance.regressionAlertTaskLedgerDriftCheckpointLedger && matchesSearch([
        "regression alert remediation task ledger drift checkpoints",
        governance.regressionAlertTaskLedgerDriftCheckpointLedger.status || "",
        governance.regressionAlertTaskLedgerDriftCheckpointLedger.markdown || "",
        ...(governance.regressionAlertTaskLedgerDriftCheckpointLedger.items || []).map((item) => `${item.title || ""} ${item.status || ""} ${item.regressionAlertTaskLedgerDriftDecision || ""} ${item.regressionAlertTaskLedgerDriftLabel || ""} ${item.regressionAlertTaskLedgerSnapshotTitle || ""}`)
      ])
      ...
      agentWorkOrderRuns: filterAndSort(
        ...
        (run) => [run.projectName || "", run.title || "", run.status || "", run.archivedAt ? "archived" : "active", run.slaBreachedAt && !run.slaResolvedAt ? "sla breached" : "", run.slaResolvedAt ? "sla resolved" : "", run.slaAction || "", String(run.slaEscalationCount || ""), run.readinessStatus || "", run.objective || "", run.blockers.join(" "), run.notes || "", run.profileTargetTaskLedgerBaselineHealth || "", run.profileTargetTaskLedgerBaselineFreshness || "", run.profileTargetTaskLedgerBaselineDriftSeverity || "", String(run.profileTargetTaskLedgerBaselineUncheckpointedDriftCount || 0), run.targetBaselineAuditLedgerBaselineHealth || "", run.targetBaselineAuditLedgerBaselineFreshness || "", run.targetBaselineAuditLedgerBaselineDriftSeverity || "", String(run.targetBaselineAuditLedgerBaselineUncheckpointedDriftCount || 0), run.regressionAlertTaskLedgerBaselineHealth || "", run.regressionAlertTaskLedgerBaselineFreshness || "", run.regressionAlertTaskLedgerBaselineRefreshGateDecision || "", run.regressionAlertTaskLedgerBaselineDriftSeverity || "", String(run.regressionAlertTaskLedgerBaselineUncheckpointedDriftCount || 0), String(run.regressionAlertTaskLedgerBaselineOpenEscalatedCheckpointCount || 0), run.history.map((event) => event.note).join(" ")],
        (left, right) => new Date(right.updatedAt || right.createdAt).getTime() - new Date(left.updatedAt || left.createdAt).getTime()
  ...
  function bindGovernanceQuickActions(container) {
    ...
    container.querySelectorAll("[data-regression-alert-task-ledger-drift-copy]").forEach((element) => {
      if (!(element instanceof HTMLButtonElement)) return;
      element.onclick = async (event) => {
        event.preventDefault();
        event.stopPropagation();
        ...
        try {
          element.disabled = true;
          element.textContent = "Copying";
          await copyLatestRegressionAlertTaskLedgerSnapshotDrift();
          element.textContent = "Copied";
        } catch (error) {
          element.textContent = originalLabel;
          alert(getErrorMessage(error));
    ...
    container.querySelectorAll("[data-regression-alert-task-ledger-snapshot-drift-id]").forEach((element) => {
      if (!(element instanceof HTMLButtonElement)) return;
      element.onclick = async (event) => {
        event.preventDefault();
        event.stopPropagation();

        const snapshotId = element.dataset.regressionAlertTaskLedgerSnapshotDriftId || "";
        ...
        try {
          ...
          const diff = await api.fetchRegressionAlertTaskLedgerSnapshotDiff(snapshotId);
    ...
    container.querySelectorAll("[data-regression-alert-task-ledger-drift-field]").forEach((element) => {
      if (!(element instanceof HTMLButtonElement)) return;
      element.onclick = async (event) => {
        event.preventDefault();
        event.stopPropagation();

        const snapshotId = element.dataset.regressionAlertTaskLedgerDriftSnapshotId || "latest";
        const status = element.dataset.regressionAlertTaskLedgerDriftStatus || "all";
        const field = element.dataset.regressionAlertTaskLedgerDriftField || "";
        const decision = element.dataset.regressionAlertTaskLedgerDriftDecision || "deferred";
        if (!field) return;
        const note = (window.prompt(`Optional non-secret Regression Alert task ledger drift checkpoint note for ${decision}`, "") || "").trim();
        const originalLabel = element.textContent || "";
        try {
          element.disabled = true;
          element.textContent = "Saving";
          const response = await api.checkpointRegressionAlertTaskLedgerDrift({
            snapshotId,
    ...
    container.querySelectorAll("[data-regression-alert-task-ledger-drift-checkpoint-ledger-copy]").forEach((element) => {
      if (!(element instanceof HTMLButtonElement)) return;
      element.onclick = async (event) => {
        event.preventDefault();
        event.stopPropagation();

        const status = element.dataset.regressionAlertTaskLedgerDriftCheckpointLedgerCopy || "all";
        const originalLabel = element.textContent || "";
        try {
          element.disabled = true;
          element.textContent = "Copying";
          const payload = await api.fetchRegressionAlertTaskLedgerDriftCheckpointLedger(status === "open" || status === "closed" ? status : "all");
    ...
    container.querySelectorAll("[data-regression-alert-task-ledger-drift-checkpoint-task-id]").forEach((element) => {
      if (!(element instanceof HTMLButtonElement)) return;
      element.onclick = async (event) => {
        event.preventDefault();
        event.stopPropagation();

        const taskId = element.dataset.regressionAlertTaskLedgerDriftCheckpointTaskId || "";
        const status = element.dataset.regressionAlertTaskLedgerDriftCheckpointTaskStatus || "";
        if (!taskId || !status) return;
        const originalLabel = element.textContent || "";
        try {
          element.disabled = true;
    ...
    container.querySelectorAll("[data-convergence-review-task-pair-id]").forEach((element) => {
      ...
      element.onclick = async (event) => {
        ...
        try {
          element.disabled = true;
          element.textContent = "Creating";
          const payload = await api.createConvergenceReviewTasks({ pairIds: [pairId], ...getCliBridgeScopeOptions() });
          await renderGovernance();
          element.textContent = payload.totals.created
  ...
  function buildGovernanceReportMarkdown() {
    ...
    lines.push(`- Regression Alert baseline capture: ${executionMetrics?.alertBaselineReviewRequired ?? 0} review | ${executionMetrics?.alertBaselineHealthy ?? 0} healthy | ${executionMetrics?.alertBaselineMissing ?? 0} missing | ${executionMetrics?.alertBaselineCaptured ?? 0} captured | ${executionMetrics?.alertBaselineRefreshGateHold ?? 0} hold gate(s) | ${executionMetrics?.alertBaselineOpenEscalatedCheckpoints ?? 0} escalated checkpoint(s)`);
    ...
    if (governance.agentWorkOrderRuns.length) {
      for (const run of governance.agentWorkOrderRuns) {
        ...
        lines.push(`  Regression Alert baseline: ${run.regressionAlertTaskLedgerBaselineHealth || "missing"} / refresh ${run.regressionAlertTaskLedgerBaselineRefreshGateDecision || "ready"} / ${run.regressionAlertTaskLedgerBaselineUncheckpointedDriftCount || 0} uncheckpointed drift item(s) / ${run.regressionAlertTaskLedgerBaselineOpenEscalatedCheckpointCount || 0} escalated checkpoint(s)`);
  ...
  async function renderGovernance() {
    ...
    try {
      ...
      const itemCount = governance.recentActivity.length
        ...
        + (mutationScopeInventory ? 1 : 0)
        + (mutationScopeInventory?.unguarded || []).length
        + (mutationScopeInventory?.items || []).filter((item) => item.scopeRelevant).length
        + (scanDiff ? 1 : 0)
        + (scanDiff?.alerts || []).length
        ...
        + (regressionAlertTaskLedgerSnapshotDiff ? 1 : 0)
  ...
  async function bootstrapGovernance(mode) {
    ...
    await api.bootstrapGovernance({ mode, projectIds, ...getCliBridgeScopeOptions() });
    await renderGovernance();
  }

  async function refreshGovernanceProfileTargets() {
    await api.refreshGovernanceProfileTargets(getCliBridgeScopeOptions());
    await renderGovernance();
  }
  ...
  async function seedGovernanceProfileTargetTasks() {
    ...
    const result = await api.createGovernanceProfileTargetTasks({ items, ...getCliBridgeScopeOptions() });
    await renderGovernance();
  ...
  async function copyLatestRegressionAlertTaskLedgerSnapshotDrift() {
    const diff = await api.fetchRegressionAlertTaskLedgerSnapshotDiff("latest");
    await copyText(diff.markdown);
    return diff.hasDrift ? `Copied ${formatDriftSeverityLabel(diff.driftSeverity)}` : diff.hasSnapshot ? "No Drift" : "No Snapshot";
  }
  ...
  return {
    ...
    copyLatestRegressionAlertTaskLedgerSnapshotDrift,
