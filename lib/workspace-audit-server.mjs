  ...
  return createServer(async (req, res) => {
    ...
    if (req.method === "GET" && pathname === "/api/releases/checkpoints/diff") {
      try {
        ...
        if (!checkpoint) {
          return sendJson(res, 200, createMissingReleaseCheckpointDriftPayload());
        }
        return sendJson(res, 200, createReleaseCheckpointDriftPayload(checkpoint, await createReleaseSummaryPayload(persisted)));
      } catch (error) {
        return sendJson(res, 500, { error: error.message });
      }
    }
